...
// M11 — the HTTP timeout must outlast a long poll ---------------------------

/// Telega long-polls `getUpdates` with a 30-second timeout by default. An HTTP
/// timeout equal to it races the response; a shorter one aborts every poll.
const polling_timeout_ms = 30_000

pub fn default_timeout_outlasts_a_long_poll_test() {
  { telega_httpc.default_timeout_ms > polling_timeout_ms } |> should.be_true
}
...
pub fn a_short_timeout_bounds_the_call_test() {
  ...
  telega_httpc.fetch_adapter_with_timeout(timeout_ms: 200)(req)
  |> should.be_error
