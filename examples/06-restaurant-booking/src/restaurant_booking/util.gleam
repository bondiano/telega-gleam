...
fn db_row_to_flow_instance(row: sql.LoadFlowInstanceRow) -> pflow.FlowInstance {
...
pub fn create_database_storage(db: pog.Connection) -> pflow.FlowStorage {
  pflow.FlowStorage(
    save: fn(instance) {
      ...
      // Use a far-future timestamp since database still expects it
      let far_future = timestamp.from_unix_seconds(2_000_000_000)
...
pub fn get_restaurant_name() -> String {
  case envoy.get("RESTAURANT_NAME") {
    Ok(name) -> name
    Error(_) -> "Bella Vista Restaurant"
  }
}

/// Generate a confirmation code
pub fn generate_confirmation_code() -> String {
  "RES" <> pad_left(int.to_string(int.random(99_999)), 5, "0")
}

/// Pad a string on the left with a character
pub fn pad_left(str: String, length: Int, pad_with: String) -> String {
  let current_length = string.length(str)
  case current_length >= length {
    True -> str
    False -> {
      let padding_needed = length - current_length
      string.repeat(pad_with, padding_needed) <> str
    }
  }
}
