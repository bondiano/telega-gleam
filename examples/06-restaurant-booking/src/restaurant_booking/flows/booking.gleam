...
pub fn create_booking_flow(
  ...
  pflow.new("booking", "welcome", storage)
  |> pflow.add_step(
    "welcome",
    pflow.message_step(
      fn(_) {
  ...
  |> pflow.add_step(
    "guests",
    pflow.text_step("👥 How many guests? (1-12)", "guest_count", "confirm"),
  )
  |> pflow.add_step("confirm", fn(ctx, instance) {
    confirm_and_book_step(db, ctx, instance)
  })
  |> pflow.on_complete(fn(ctx, _) { Ok(ctx) })
  |> pflow.on_error(fn(ctx, _, _) {
    reply.with_text(ctx, "❌ Booking error. Use /book to try again.")
    |> result.map(fn(_) { ctx })
    |> result.map_error(fn(_) { "Error message failed" })
  })
}
...
Thank you for choosing " <> util.get_restaurant_name() <> "!
              ...
              case reply.with_text(ctx, success_message) {
                ...
                Error(_) ->
                  Ok(#(ctx, pflow.StandardAction(flow.Cancel), instance))
              }
            }
            Error(err) -> {
              case reply.with_text(ctx, "❌ Booking failed: " <> err) {
    ...
    Error(error_msg) -> {
      case reply.with_text(ctx, "❌ Error: " <> error_msg) {
        Ok(_) -> Ok(#(ctx, pflow.StandardAction(flow.Cancel), instance))
        Error(_) -> Ok(#(ctx, pflow.StandardAction(flow.Cancel), instance))
      }
    }
...
fn create_booking(
  ...
  let user_id = ctx.update.from_id
